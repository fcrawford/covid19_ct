...
if(scenario_name == "fast") {
  ...
  mydistancing_stepdown_dates = seq(mydistancing_on_date+1, mydistancing_on_date+1+days(14*9), by=14)
} else {
  str = "\nwith release on 5/20 and monthly increments in contact"
  mydistancing_stepdown_dates = seq(mydistancing_on_date+1, mydistancing_on_date+1+days(30*9), by=30)
}


## choose your scenario and get state0, params and a sample from posterior

# scenario 1: low asymptomatic: q_A = 0.36, mean(q_Is) = 0.064
if(case_name == "low"){
...
# scenario 2: medium asymptomatic: q_A = 0.5, mean(q_Is) = 0.05
if(case_name == "medium"){
...
# scenario 3: high asymptomatic: q_A = 0.7, mean(q_Is) = 0.03
if(case_name == "high"){
...
myposterior$distancing_effect = myposterior$lockdown_effect
