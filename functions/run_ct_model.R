...
init = matrix(0, ncol=8, nrow=8)
compartments <- c("E", "I_s", "I_m", "A", "H", "D", "R" )
...
for (k in 1:length(compartments)){
 ...
 init[,k] = comp.init 
  ...
  colnames(init) <- c("E", "Is", "Im", "A", "H", "D", "R", "county")
...
params$k_Is_ins = par_smpl$k_Is_ins
params$gamma_H = par_smpl$gamma_H
params$gamma_Hbar = par_smpl$gamma_H
   
params$m_H = par_smpl$m_H
params$q_ins = par_smpl$q_ins
params$m_Is_mult = par_smpl$m_Is_mult
